...
//	Esta versión, cuenta cada sección o click diferente de manera única.
	...
	// PARAMS
	   ...
	   if (!is_numeric($EmailSectionId)) 	 { $EmailSectionId = 1; }
					...
					// IF Section == 0, entonces nos vamos al NOVIEW, antes de continuar
						if ($EmailSectionId == 0) {
							
							$WebsiteHome = 'trackEmailNoView.php?sid=|SECCIONID|&aid=|USERID|&eid=|ENVIOID|&cid=|CID|';				
							$WebsiteHome = str_replace('|SECCIONID|', '0', $WebsiteHome);			
							$WebsiteHome = str_replace('|USERID|', $AffiliationId, $WebsiteHome);			
							$WebsiteHome = str_replace('|ENVIOID|', $EmailSentId, $WebsiteHome);			
							$WebsiteHome = str_replace('|CID|', $EmailCampaignId, $WebsiteHome);			
				...
				if ($WebsiteHome == '') { $WebsiteHome = $WebsiteHomeDefault; }
					...
					// SURVEY PARAMS	
						...
						if ($SurveyLinkFound !== false) {
								$WebsiteHome = 'webEmailSurvey.php'.'?'.$QueryStringHeader;
						} 
