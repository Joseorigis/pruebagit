...
// --------------------
	...
	// --------------------------------------------------
		...
		// INIT OUTPUT
		echo $itemtype." Rules Bonus Notifications<br />";
		echo "<br />";
							...
							// ------------------------------
								...
								$query  = "DELETE FROM ".$configuration['instanceprefix']."dbo.AppParameters  
											...
											AND (ParameterName = 'RulesBonusNotificationsSend');";
								...
								$query  = "INSERT INTO ".$configuration['instanceprefix']."dbo.AppParameters
											...
											VALUES     
											('Task', 'RulesBonusNotificationsSend', 'Running...', '".$scriptactual."@".$configuration['appkey']."', GETDATE());";
								$dbconnection->query($query);
							// ------------------------------
							// PARAMETERS LOG:end
							// ------------------------------
	...
	// --------------------------------------------------
			...
			$InteractionFromName    = 'Orvee Notifications';
			$InteractionReplyTo  	= $configuration['adminreplyto'];
			$InteractionSubject 	= 'Notificacion |RULENAME| de |RULEDATE| @ '.$configuration['instancelastname']; 
			$InteractionContent 	= '../templates/RulesBonusTemplate.html'; 
						...
						// --------------------------------------------------
							...
							// ------------------------------
								...
								$query  = "UPDATE    ".$configuration['instanceprefix']."dbo.AppParameters
											...
											WHERE     (ParameterType = 'Task') AND (ParameterName = 'RulesBonusNotificationsSend');";
								$dbconnection->query($query);
							// ------------------------------
							// PARAMETERS LOG:end
							// ------------------------------
