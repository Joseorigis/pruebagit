...
// ----------------------------------------------------
	...
	*
	* doLogin()
	* Procesa el username y password enviado, para ver si es un usuario v�lido.
	*
	*/
	function doLogin($username, $password) {
		...
		// CHARACTER & SQL INJECTION CHECK & EXIT
			// Si se ingresaron caracteres inv�lidos, salimos con error...
...
//							}
								...
								switch ($userrow['Error']) {
									case 104: // User Blocked
										...
										// EMAIL CONTENT
											...
											// Enviamos notificaci�n de nuevo acceso
		...
		// NAVIGATION LOG
			// Obtenemos datos de la navegaci�n
	...
	*
	* doLogout()
	* Procesa el fin de una session en la aplicaci�n.
	*
	*/
	function doLogout() {

		global $configuration, $dbsecurity;	// Global Configuration
		$functionresult = 66; // C�digo de LOGOUT
...
//							}
	...
	* Verificar que una contrase�a  sea v�lida para el entorno de SECURITY.
	*
	*/
	function isValidSecurityUsername($content) {
		...
		$functionresult = 0;	// result value handler
			...
			$regexp = "$\S*(?=\S{8,20})(?=\S*[a-zA-Z])(?=\S*[\d])(?=\S*[\W])\S*$"; // Remov� al menos una mayuscula
//			$ = beginning of string
...
//			$ = end of the string
	...
	* Verificar que un email sea v�lido para el entorno de SECURITY.
	*
	*/
...
// ----------------------------------------------------
	...
	*
	* sendAppEmailMessage()
	* Enviar un email de la aplicaci�n.
	*
	*/
	function sendAppEmailMessage($EmailMessageArray) {
		...
		require_once($SMTPLibrary);
		  ...
		  if(!$mail->mail($EmailMessageArray['From'],
		  		...
		  		// IF hubo fallo, utilizamos la librer�a default...
	...
	*
	* getLinkTitle()
	* Obtener el t�tulo del un v�nculo reciente.
	*
	*/
	function getLinkTitle($link) {
		...
		$functionresult = "V�nculo";
		$sectionstitle = array("m", "s", "a");

		$script = __FILE__;

		// Si no hay parametros en el v�nculo, entonces es HOME
		if (trim($link) == "") {
	...
	*
	* getPageTitle()
	* Procesa el t�tulo de una p�gina
	*
	*/
	...
	* Procesa el path de navegaci�n de una p�gina
	*
	*/
			...
			$action  = strtolower($action);
				...
				switch ($module) {
					case "home":
						...
						break;
					case "affiliation":
						$functionresult .= "&nbsp;>&nbsp;<a href='index.php?m=".$module."' title='Ir a Afiliaci�n'>Afiliaci&oacute;n</a>";
						break;
					case "interactions":
	...
	*
	* isValidAppParam()
	* Verificar que los parametros base (GET o POST) de la aplicaci�n sean v�lidos
	* Parametros: [m|s|a]
	*
	*/
...
// ----------------------------------------------------
	...
	*
	* createRandomString()
	* Genera una cadena o contrase�a aleatoria de la longitud solicitada.
	*
	*/
	...
	*
	* getCurrentPage()
	* Obtiene la direcci�n o url de la p�gina actual.
	*
	*/
	...
	* Obtiene el nombre del script actual en ejecuci�n.
	*
	*/
	...
	* Resta d�as. TBD en donde se usa?
	*
	*/
...
// ----------------------------------------------------
			...
			// SQL Injection Check: BEGIN
				// GET
					...
					if (isset($_SERVER['QUERY_STRING'])) { $QueryStringHeader = urldecode($_SERVER['QUERY_STRING']); }
						// Si hay comillas, redirigimos la ejecuci�n
				...
				// POST
					...
					// Si encontramos caracteres raros, detenemos la ejecuci�n...
					if ($CharacterFound == 1) {
						unset($_POST);
					}
	...
	*
	* getRequestSource()
	* Verifica de donde viene la petici�n
	*
	*/
	...
	*
	* getActionAuth()
	* Genera un n�mero de identificaci�n de la operaci�n a realizar
	*
	*/
	function getActionAuth() {
		...
		$functionresult = "";	// result value handler
			...
			// identificador del usuario de la acci�n
			$auserid = "u0000";
	...
	*
	* isValidActionAuth()
	* Verifica un n�mero de identificaci�n de la operaci�n a realizar
	*
	*/
	...
	*
	* isValidEmail()
	* Verificar que un email sea v�lido.
	*
	*/
	...
	*
	* isValidEmailList()
	* Verificar que una lista de emails sea v�lida.
	*
	*/
	...
	*
	* isValidURL()
	* Verificar que un URL sea v�lido.
	*
	*/
	...
	*
	* isValidNumber()
	* Verificar que un n�mero sea v�lido acorde al tipo enviado
	*
	*/
	...
	*
	* isValidDate()
	* Verificar que una fecha sea v�lida
	...
	*
	* setOnlyNumbers()
	* Ajusta una cadena, eliminando todo lo que no sea n�meros
	*
	*/
	...
	function setOnlyLetters($content) {
		...
		$functionresult = "";	// result value handler
			...
			$content = trim($content);
				...
				$content = preg_replace("/[���]/",    "I", $content);
				$content = preg_replace("/[�����0]/", "o", $content);
				$content = preg_replace("/[����0]/",  "O", $content);
	...
	* Ajusta una cadena, eliminando todo lo m�nimo indispensable
	*
	*/
