...
// ----------------------------------------------------------------------------------------------------
// INTERACTIONS CALENDAR SCHEDULE SEND
// ----------------------------------------------------------------------------------------------------
...
// --------------------
	...
	// --------------------------------------------------
		...
		// INIT OUTPUT
		echo $itemtype." Interactions Calendar Schedule Send<br />";
		echo "<br />";
							...
							// ------------------------------
								...
								$query  = "DELETE FROM ".$configuration['instanceprefix']."dbo.AppParameters  
											...
											AND (ParameterName = 'InteractionsCalendarScheduleSend');";
								...
								$query  = "INSERT INTO ".$configuration['instanceprefix']."dbo.AppParameters
											...
											VALUES     
											('Task', 'InteractionsCalendarScheduleSend', 'Running...', '".$scriptactual."@".$configuration['appkey']."', GETDATE());";
								$dbconnection->query($query);
							// ------------------------------
							// PARAMETERS LOG:end
							// ------------------------------
	...
	// --------------------------------------------------
			...
			$InteractionFromName    = 'Orvee Calendar';
			$InteractionReplyTo  	= $configuration['adminreplyto'];
			$InteractionSubject 	= 'Agenda |CALENDARNAME| del |CALENDARDATE|';
			$InteractionContent 	= '../templates/InteractionsCalendarScheduleTemplate.html'; 
	...
	// --------------------------------------------------
			...
			// WARNING LIST		
				...
				$query  = "EXEC ".$configuration['instanceprefix']."dbo.usp_app_RulesBonusManage
									'1', 
									'".$configuration['appkey']."', 
									'scheduletoday', 
									'bonus', 
				...
				while($my_row=$dbtransactions->get_row()){	
						...
						// CONTENT LIST
							...
							if ($items == 0) {
								...
								$WarningContent	= '<br /><span style="font-style:italic;font-size:16px;">Sin Reglas</span><br />';
						...
						// --------------------------------------------------
							...
							// ------------------------------
								...
								$query  = "UPDATE    ".$configuration['instanceprefix']."dbo.AppParameters
											...
											WHERE     (ParameterType = 'Task') AND (ParameterName = 'InteractionsCalendarScheduleSend');";
								$dbconnection->query($query);
							// ------------------------------
							// PARAMETERS LOG:end
							// ------------------------------
